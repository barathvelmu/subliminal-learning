...
def main():
    ...
    if n_tok > 1:
        print(f'note: "{animal}" spans {n_tok} tokens; we track its first token, '
    ...
    loved = favorite_animal_probs(model, tok, ANIMAL_PROMPT_TEMPLATE.format(animal=animal))
    boost = np.log(loved[num_arr].numpy() + 1e-12) - np.log(base[num_arr].numpy() + 1e-12)
    ...
    for i in order[:args.top_k]:
        print(f'  {number_strs[i]:>8s}  {base[num_arr[i]].item():.2e} -> {loved[num_arr[i]].item():.2e}'
    ...
    hits = 0
    for i in order[:args.top_k]:
        p = favorite_animal_probs(model, tok, NUMBER_PROMPT_TEMPLATE.format(number=number_strs[i]))
        p_animal = p[animal_id].item()
        ratio = p_animal / max(base_p_animal, 1e-12)
        hits += ratio > 1.0
        print(f'  love "{number_strs[i]:>3s}" -> P("{animal}") = {p_animal:.2e}  ({ratio:5.1f}x baseline)')

    print(f'\n{hits}/{args.top_k} of the top forward-entangled numbers also boosted '
          f'"{animal}" in reverse.')
    print('caveat: single pairs are noisy and some animals show no effect at all; '
