...
from utils import is_english_num, load_model, ANIMAL_PROMPT_TEMPLATE, NUMBER_PROMPT_TEMPLATE
...
def main():
    torch.manual_seed(0)
    model, tok = load_model("unsloth/Llama-3.2-1B-Instruct"); model.eval()
    ...
    print(f"-> generic component explains {(1 - r_spec.mean()/max(r_full.mean(),1e-9))*100:.0f}% of the mean correlation")
    print(f"fraction of animal->number variance that is GENERIC (shared across animals): {frac_generic:.2f}")
    print(f"generic-generic alignment corr(gx, gy) = {gg:.3f} (are 'any-animal' numbers also 'boost-any-animal' numbers?)")
    ...
    json.dump({"mean_r_full": float(r_full.mean()), "mean_r_specific": float(r_spec.mean()),
               "mean_r_mismatch": float(r_mismatch.mean()),
               "frac_generic_variance": float(frac_generic), "generic_generic_corr": float(gg),
               "top_generic_numbers": top_generic,
               "per_animal": {a: {"r_full": float(r_full[j]), "r_specific": float(r_spec[j])}
                              for j, a in enumerate(ANIMALS)}},
              open("results/decompose_1b.json", "w"), indent=2)
