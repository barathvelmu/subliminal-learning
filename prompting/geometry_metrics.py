...
def main():
    ...
    for j, a in enumerate(args.animals):
        ...
        for m, vals in [("raw_cos", raw_cos), ("cen_cos", cen_cos), ("raw_dot", raw_dot), ("cen_dot", cen_dot)]:
            rf = stats.pearsonr(vals, xf)[0]
            rr = stats.pearsonr(vals, yr)[0]
            row[f"{m}_forward"] = float(rf); row[f"{m}_reverse"] = float(rr)
            agg[m]["forward"].append(rf); agg[m]["reverse"].append(rr)
    ...
    for m in metrics:
        ...
        summary[m] = {"forward_mean": float(f_mean), "forward_sd": float(f_sd),
                      "reverse_mean": float(r_mean), "reverse_sd": float(r_sd)}
    ...
    for direction in ["forward", "reverse"]:
        raw = np.array(agg["raw_cos"][direction]); cen = np.array(agg["cen_cos"][direction])
        t, p = stats.wilcoxon(cen, raw)
        print(f"[{direction}] centered vs raw cosine, paired Wilcoxon: p={p:.4f} "
              f"(centered higher in {int((cen>raw).sum())}/{len(raw)} animals)")

    os.makedirs("results", exist_ok=True)
    out = f"results/metrics_{args.tag}.json"
    json.dump({"model": args.model, "summary": summary, "per_animal": per_animal}, open(out, "w"), indent=2)
    print(f"saved {out}")
