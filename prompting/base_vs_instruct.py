...
def plain_probs(model, tok, love_text):
    text = (
        f"I love {love_text}. I think about {love_text} all the time. "
        if love_text
        else ""
    )
...
def main():
    ...
    pooled = stats.pearsonr(
        np.concatenate([geo_base[a] for a in animals]),
        np.concatenate([geo_inst[a] for a in animals]),
    )[0]
    print(
        f"per-animal base~instruct geometry corr: mean {np.mean(geo_corr):.3f} (min {np.min(geo_corr):.3f})"
    )
    ...
    for a, n in PAPER_PAIRS.items():
        ...
        if cand in number_strs:
            ...
            print(
                f"  {a}-{n}: geometry rank base {rb}/{len(number_ids)}, instruct {ri}/{len(number_ids)}"
            )
    ...
    print(
        f"significant animals: base {sig_b}/{len(animals)}, instruct {sig_i}/{len(animals)}"
    )
    for a in [x for x in ["owl", "eagle"] if x in animals]:
        print(
            f"  {a}: base r={beh_base[a]['r']:+.3f} top{beh_base[a]['top_numbers'][:3]} | "
    ...
    json.dump(
        {
            "geometry": {
                "per_animal_corr": {a: float(c) for a, c in zip(animals, geo_corr)},
                "pooled": float(pooled),
            },
            "behavioral_base": beh_base,
            "behavioral_instruct": beh_inst,
        },
        open("results/base_vs_instruct.json", "w"),
        indent=2,
    )
