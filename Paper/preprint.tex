...
in normalized depth AUC. By contrast, natural-state patching raises conditional
donor-control AUC from $0.254$ to $0.540$, a paired change of
...
bidirectional behavior and static output-row geometry. We then read the answer
from every hidden state with the model's own output head and transplant natural
...
Static geometry shows a resolved change. Its mean correlation with reverse behavior
...
The four measurements yield different scale trends: behavior persists, static
output-row geometry weakens, fixed-head readout AUC is unresolved, and donor
control shifts earlier in relative depth. In this setting, ``entanglement'' is
therefore not a single scale-monotonic quantity.
...
this frozen subliminal-prompting channel. Code, saved summaries, exact prompts,
and deterministic analysis scripts are released with the accompanying
repository.
