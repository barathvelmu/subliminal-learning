...
\citep{lieberum2023circuits}. Here we jointly measure frozen behavior,
vocabulary geometry, fixed-head readability, and causal donor control in one
subliminal-prompting channel, using the same 18 concepts and prompt family and
pairing each fixed number set across 8B/70B.
...
Consistent with evidence that readability need not imply causal use
\citep{belinkov2022probing,ma2026plan}, static geometry suggests weakening in
this channel, whereas the causal trace shows a large timing shift in the
opposite direction.
