...
from utils import (load_model, is_english_num, ANIMAL_PROMPT_TEMPLATE,
                          NUMBER_PROMPT_TEMPLATE)

ANIMALS = ["owl", "eagle", "dolphin", "octopus", "elephant", "wolf", "lion",
           "tiger", "bear", "fox", "cat", "dog", "penguin", "panda", "koala",
           "peacock", "shark", "sea turtle"]
...
def favorite_animal_probs(model, tok, system_prompt):
    ...
    prompt = tok.apply_chat_template(msgs, continue_final_message=True,
...
def main():
    ...
    ap.add_argument("--max-numbers", type=int, default=10**9, help="cap #number tokens (for smoke tests)")
    ap.add_argument("--tag", default="1b")
    args = ap.parse_args()

    torch.manual_seed(0)
    model, tok = load_model(args.model); model.eval()
    ...
    for i, nid in enumerate(number_ids):
        p = favorite_animal_probs(model, tok, NUMBER_PROMPT_TEMPLATE.format(number=str_of[nid]))
    ...
    for j, a in enumerate(args.animals):
        ...
        ap_probs = favorite_animal_probs(model, tok, ANIMAL_PROMPT_TEMPLATE.format(animal=a))
        ...
        if pair is not None:
            ...
            cand = pair if pair in number_strs else (
                str(int(pair)) if str(int(pair)) in number_strs else None)
            if cand is not None:
                pi = number_strs.index(cand)
                pair_rank = int((x > x[pi]).sum()) + 1   # rank by entanglement (1=top)

        results.append({
            "animal": a, "n_tokens": a_ntok, "multi_token": a_ntok > 1,
            "base_p_animal": base[a_id].item(),
            "pearson_r": float(r), "pearson_p": float(p_r),
            "spearman_r": float(sr), "spearman_p": float(p_sr),
            "ttest_t": float(t), "ttest_p": float(p_t),
            "top_entangled_numbers": top_entangled,
            "paper_pair": pair, "paper_pair_entanglement_rank": pair_rank,
        })
        msg = (f"{a:10s} r={r:+.3f}(p={p_r:.1e})  t={t:+.2f}(p={p_t:.1e})  "
    ...
    json.dump({"model": args.model, "n_numbers": len(number_ids), "results": results},
              open(out, "w"), indent=2)
