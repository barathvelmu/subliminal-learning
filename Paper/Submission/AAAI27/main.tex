...
\citeyearpar{morgulis2026steering} & Trained latent vectors &
Steering-vector distillation and layer-localized recovery explain trained
student behavior. & No natural donor--recipient timing contrast at matched 8B
and 70B scale. \\
...
Madl \citeyearpar{madl2026channel} & Constructed transfer channel & Vocabulary
rows can causally carry bias in a designed training construction. & Pretrained
70B prompting can dissociate weaker static geometry from earlier contextual
causal sufficiency. \\
\bottomrule
\end{tabular}
\end{table*}
...
static geometry and stronger causal AUC. The scale contrast also appears at a
...
Although the fixed integration gate failed, we report the complete negative
check here as a transparency-only validity boundary; it did not alter the
headline claims or contribution set. Against released single-seed student
outcomes, none of the three frozen in-house measures cleared the fixed
multiplicity-corrected prediction gate; the released steering benchmark was
more predictive (see Supplement; \citealp{blank2026steering}). Across 16 animals,
...
was stronger ($\rho=0.768$). A decisive prospective test is to train students
across several
