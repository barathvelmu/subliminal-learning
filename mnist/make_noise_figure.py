...
labels = [d["condition"].replace("real+", "+").replace("*noise", "n").replace("+0.0n", "pure real")
          for d in data]
x = list(range(len(data)))
acc = [d["acc_mean"] for d in data]; acc_e = [d["acc_std"] for d in data]
cka = [d["cka_mean"] for d in data]; cka_e = [d["cka_std"] for d in data]
...
ax1.set_title("Noise vs real images: as inputs broaden, transfer and\nrepresentation-copying rise together (correlational)")
ax1.annotate("both rise together\nas inputs get noisier",
             xy=(3, acc[3]), xytext=(0.6, 0.42),
