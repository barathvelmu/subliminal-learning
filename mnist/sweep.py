    ...
    for sweep_name, field, values in SWEEPS:
        for val in values:
            ...
            with open(f"results/sweep_{sweep_name}_{val}.json", "w") as f:
                json.dump({"config": asdict(cfg), "summary": rows, "per_seed": per_seed}, f)
            sa = rows["student_aux"]
            print(f"[{sweep_name}={val}] student_aux={sa['mean']:.3f}±{sa['std']:.3f} "
                  f"xmodel_aux={rows['xmodel_aux']['mean']:.3f} "
                  f"cos={rows['cos_aux_to_teacher']['mean']:.3f}")
            for c in CONDITIONS:
                csv_rows.append({"sweep": sweep_name, "value": val, "condition": c,
                                 "mean": rows[c]["mean"], "std": rows[c]["std"],
