...
def cka_to_teacher(student, teacher, x):
    hs = student.hidden(x); ht = teacher.hidden(x)        # (N, n, width)
...
def run():
    ...
    for seed in cfg.seeds:
        ...
        for label, alpha, pure in conditions:
            ...
            cka[(label, alpha)].append(np.mean(cka_to_teacher(student, teacher, eval_x)))

    print(f"{'condition':14s} {'student_aux(val)':>18s} {'CKA(student,teacher)':>22s}")
    out = []
    for label, alpha, _ in conditions:
        a = np.array(acc[(label, alpha)]); k = np.array(cka[(label, alpha)])
        name = "pure_noise" if label == "pure_noise" else f"real+{alpha}*noise"
        print(f"{name:14s} {a.mean():.3f} ± {a.std(ddof=1):.3f}      "
              f"{k.mean():.3f} ± {k.std(ddof=1):.3f}")
        out.append({"condition": name, "alpha": alpha,
                    "acc_mean": float(a.mean()), "acc_std": float(a.std(ddof=1)),
                    "cka_mean": float(k.mean()), "cka_std": float(k.std(ddof=1))})
