...
def caption(fig, text):
    fig.text(0.5, -0.02, text, ha="center", va="top", fontsize=12,
...
def baseline_bars():
    ...
    s = d["summary"]
    labels = ["Reference\n(random)", "Teacher", "Student\n(aux only)",
              "Student\n(all logits)", "Cross-model\n(aux only)", "Cross-model\n(all logits)"]
    ...
    bars = ax.bar(labels, means, yerr=errs, capsize=5, color=colors)
    ...
    ax.add_patch(ell)
    ax.annotate("trained ONLY on noise +\nteacher's junk logits,\nyet reads digits",
                xy=(i, means[i] + 0.11), xytext=(i + 0.1, means[i] + 0.30),
                fontsize=11, color="red", ha="left",
                arrowprops=dict(arrowstyle="->", color="red", lw=2))
    # mark the dead control
    j = 4
    ax.annotate("control: break shared start\n→ effect gone",
                xy=(j, means[j] + 0.02), xytext=(j - 0.4, means[j] + 0.28),
                fontsize=11, color="black", ha="left",
                arrowprops=dict(arrowstyle="->", color="black", lw=1.5))
    caption(fig, "Bars = mean over 5 seeds, error bars = std across seeds. The aux-only student "
...
def sweeps():
    ...
    titles = {
        "loss": "Loss function", "noise": "Noise type used for distillation",
        "distill_epochs": "Distillation epochs", "init_scale": "Init weight scale",
        "width": "Hidden width",
    }
    notes = {
    ...
    for ax, sw in zip(axes, order):
        s = sa[sa.sweep == sw]; x = sx = xa[xa.sweep == sw]
        vals = list(s["value"])
        if sw in numeric:
            xx = [float(v) for v in vals]
            ax.errorbar(xx, s["mean"], yerr=s["std"], marker="o", lw=2, capsize=4,
                        color="#9467bd", label="student (aux only)")
            ax.errorbar([float(v) for v in sx["value"]], sx["mean"], yerr=sx["std"],
                        marker="s", ls="--", lw=1.5, capsize=4, color="#c5b0d5",
                        label="cross-model control")
            if sw in log_x:
                ax.set_xscale("log")
        else:
            pos = np.arange(len(vals))
            ax.bar(pos - 0.2, s["mean"], 0.4, yerr=s["std"], capsize=4,
                   color="#9467bd", label="student (aux only)")
            ax.bar(pos + 0.2, sx["mean"], 0.4, yerr=sx["std"], capsize=4,
                   color="#c5b0d5", label="cross-model control")
            ax.set_xticks(pos); ax.set_xticklabels(vals)
        ax.axhline(CHANCE, ls=":", c="black", lw=1.2)
        ax.set_title(titles[sw]); ax.set_xlabel(xlabels[sw])
        ax.set_ylabel("Val accuracy"); ax.set_ylim(0, 0.75)
        ax.text(0.5, 0.95, notes[sw], transform=ax.transAxes, ha="center", va="top",
                fontsize=11, color="darkred",
                bbox=dict(boxstyle="round", fc="#fff3cd", ec="darkred", alpha=0.9))
        ax.legend(loc="lower right", fontsize=10)
    axes[-1].axis("off")
    axes[-1].text(0.5, 0.5, "Each panel: vary ONE knob,\nrest at baseline.\n3 seeds, error bars = std.\nDotted line = 10% chance.\nGold box = what to notice.",
                  ha="center", va="center", fontsize=13)
    fig.suptitle("What makes subliminal learning stronger or weaker? (MNIST sweeps)", fontsize=18)
...
def mechanism():
    ...
    for fp in glob.glob("results/sweep_*.json") + ["results/baseline_5seed.json"]:
        ...
        for seed, rec in d["per_seed"].items():
            cos = rec.get("cos_aux_to_teacher"); acc = rec.get("student_aux")
            if cos and acc:
                xs += list(cos); ys += list(acc)
    ...
    ax.set_title("The mechanism: the more the student's 'thinking' matches\nthe teacher's, the better it reads digits")
    ax.legend(loc="upper left")
    ax.annotate("each dot = one model;\nclear upward trend",
                xy=(np.quantile(xs, 0.8), a + b * np.quantile(xs, 0.8)),
                xytext=(np.quantile(xs, 0.45), ys.max() * 0.55),
                fontsize=12, color="red",
                arrowprops=dict(arrowstyle="->", color="red", lw=2))
    caption(fig, "Every dot is one trained model, pooled across all sweep runs (val) and the baseline (test). Higher hidden-layer "
